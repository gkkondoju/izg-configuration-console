...
import { Typography , CardHeader , Card, CardContent, Divider, Chip} from "@mui/material";
...
const timeline = (data) => (
  <>
    ...
    {data.map((item,index) => (
      ...
      <TimelineOppositeContent sx={{
        ...
        <TimelineSeparator>
          {index === 0 ? <TimelineDot color= "primary"/> : <TimelineDot sx={{margin: '16px 0'}}  variant="outlined"/>}
        ...
        <Typography gutterBottom variant="body1" component="div">
          <Chip label="Success" variant="outlined" color= "primary" sx={{borderRadius: 1}}/>
      </Typography>
      </TimelineItem>
      ))}
    </Timeline>
 ...
 const ChangeHistory = (props: ChangeHistoryProps) => {
  ...
  const {loading, error, data} = useQuery(AUDIT_TRAIL , {
    variables : {destId: props.destId , table: "destinations", user: session?.user?.name || undefined}
  });
  ...
  return (
  <div>
  <Card sx={{ marginTop:4,  borderRadius: '0px 0px 16px 16px'}} id="change-history">
    <CardHeader sx={{
    ...
    >
      ...
      <Divider />
      <CardContent >
      {historyDataLength > 0 ? timeline(defaultChangeHistoryView) : <p> There is no Change History at this time </p>}
    </CardContent>
    </Card>
  </div>
);
