...
import { Typography , Button, CardHeader , Card, CardContent, Divider, Chip} from "@mui/material";
    ...
    {data.map((item,index) => (
      ...
      <TimelineOppositeContent sx={{
        ...
        <Typography  variant="body2">
          {item.ran_at ? new Date(item.ran_at).
...
const TestHistory = (props: TestHistoryProps) => {
  ...
  return (
  <div>
  <Card sx={{ marginTop:4,  borderRadius: '0px 0px 16px 16px'}} id="test-history">
    <CardHeader sx={{
    ...
    subheader={
      ...
      <Divider />
      <CardContent >
      {historyDataLength > 0 ? timeline(defaultTestHistoryView) : <p> There is no Test History at this time </p>}
    </CardContent>
    </Card>
  </div>
);
